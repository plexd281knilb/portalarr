...
import { getEnabledArrInstances, getArrProfilesAndFolders, searchRadarrMovies, addRadarrMovie, getRadarrQueue, forceImportRadarrQueueItem } from "@/app/arr-actions"
...
export default function RadarrPage() {
    ...
    useEffect(() => {
        getEnabledArrInstances("radarr").then(apps => {
            setInstances(apps)
            if (apps.length > 0) {
    ...
    useEffect(() => {
        if (selectedAppId) {
            getArrProfilesAndFolders(selectedAppId, "radarr").then(data => {
    ...
    const fetchQueue = async () => {
        ...
        try {
            const data = await getRadarrQueue(selectedAppId);
    ...
    const handleSearch = async (e: React.FormEvent) => {
        ...
        setSearching(true)
        try {
            const results = await searchRadarrMovies(selectedAppId, searchTerm)
    ...
    const handleAdd = async (movie: any) => {
        ...
        try {
            await addRadarrMovie(selectedAppId, movie, parseInt(selectedProfileId), selectedFolderId)
    ...
    const handleForceImport = async (downloadId: string) => {
        ...
        try {
            await forceImportRadarrQueueItem(selectedAppId, downloadId);
        ...
        <div className="space-y-6 max-w-6xl mx-auto pb-12">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">Self-serve movie downloads and queue management.</p>
                </div>
                {instances.length > 1 && (
                                        ...
                                        <div className="flex flex-col flex-1 min-w-0 py-1">
                                            ...
                                            <div className="mt-2 flex items-center justify-between">
                                                ...
                                                <Button 
                                                    ...
                                                    disabled={addingMovieId === movie.tmdbId || movie.added}
                                                    variant={movie.added ? "secondary" : "default"}
                                                    className="h-7 text-xs"
                                                >
                                                    {addingMovieId === movie.tmdbId ? <Loader2 className="h-3 w-3 animate-spin" /> : movie.added ? "Already Added" : <><Plus className="h-3 w-3 mr-1" /> Add Movie</>}
                                                </Button>
                                            </div>
                                        </div>
                                    </div>
