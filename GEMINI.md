...
- **Tech Stack:**
  ...
  - **Wizarr Rebuilt & Automated Membership Subscriptions:** Frictionless `/join` onboarding portal, automated Plex server friend invites, default library access pre-assignment, Discord community server auto-join, and guided client setup wizards (Apple TV, Roku, Fire TV, Smart TV, Google TV, Mobile, Web). Native P2P payment routing (PayPal, Venmo, Cash App, Zelle), pure-TS SVG QR code generator (`PaymentQrModal`), cadence switcher ($180/yr vs $17.50/mo), pre-filled reconciliation memo tags (`#PORTALARR-USER-DATE`), automated IMAP payment scraping, customizable grace periods (`subscriptionGracePeriodDays`), automated access suspension upon non-payment, referral rewards ("Earn 1 Free Month per converted friend"), granular membership tiers (`STANDARD`, `PREMIUM_4K`, `VIP_ALL_ACCESS`, `FAMILY`), dedicated kid & living room profiles, and self-service notification & email preference controls.
  - **Upcoming Roadmap:**
    - **Step 1 — Autonomous AI Support Agent & Deep Server Diagnostics:** In-app first-response conversational support bot with autonomous diagnostic tools (stream health fixes, downloading replacement language audio tracks, fixing/sending book downloads, 1-click ticket escalation with auto-attached telemetry/logs), combined with deep Plex/Tautulli/Arr server log ingestion and correlated incident analysis.
    - **Step 2 — Native Uptime & Service Health Monitoring Engine (Uptime Kuma / Ping Replacement):** Multi-protocol heartbeat polling (HTTP/HTTPS, TCP/UDP port pings, SSL cert expiry), instant outage alerting (SMTP email, Discord rich embed cards, Web Push), downtime debouncing, and public/admin latency & uptime percentage status dashboards.
    - **Step 3 — Smart Bandwidth Shaper & Transcode Governor:** Dynamic download throttling when remote streams spike, NVENC/CPU load balancing, and Direct Play client coaching.
    - **Step 4 — Native IPTV & Live TV Stream Manager (xTeVe / Threadfin Replacement):** M3U/XMLTV channel bouquet management, stream health auto-failover, and virtual HDHomeRun tuner emulation for Plex Live TV & DVR.
    ...
    - **Step 6 — Native Subtitle Engine & Auto-Sync (Bazarr Replacement):** Multi-provider subtitle hunting (OpenSubtitles, Subscene, Subdl), AI audio-sync waveform alignment, and user self-service "Fix Subtitles" from My Plex Hub.
