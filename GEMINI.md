...
- **Tech Stack:**
  ...
  - **Upcoming Roadmap:**
    ...
    - **Step 3 — User Onboarding, Membership Subscriptions & Authorization (Wizarr Replacement):** Rebuilt Wizarr onboarding portal, automated Stripe/PayPal recurring billing (Annual discount vs Monthly fee with premium), referral rewards (+1 free month per referral), tiered membership perks (IPTV / Live TV / 4K transcode access), granular shelf visibility, and dedicated Kid / Living Room shared profiles.
    - **Step 4 — Autonomous AI Support Agent & Deep Server Diagnostics:** In-app first-response conversational support bot with autonomous diagnostic tools (stream health fixes, downloading replacement language audio tracks, fixing/sending book downloads), combined with deep Plex/Tautulli/Arr server log ingestion and correlated incident analysis.
